...
impl Shape {
    ...
    const CELL_SETS: [[CellSet; 9]; 3] = {
        ...
        const fn cell_sets(shape: Shape) -> [CellSet; 9] {
            ...
            while house < 9 {
                cell_sets[house] = CellSet::from::<9>(&Shape::CELLS[shape as usize][house]);
                house += 1;
            }
            cell_sets
        }
        ...
        cell_sets(Shape::Row);
        cell_sets(Shape::Column);
        cell_sets(Shape::Block);
        sets
    };

    pub const fn cells(self, house: Coord) -> CellSet {
        Shape::CELL_SETS[self as usize][house.usize()]
    }

    pub const fn cell(self, house: Coord, coord: Coord) -> Cell {
        Shape::CELLS[self as usize][house.usize()][coord.usize()]
    }

    pub const fn cell_list(self, house: Coord) -> [Cell; 9] {
        Shape::CELLS[self as usize][house.usize()]
    }
}
