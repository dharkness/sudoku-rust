...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            Error::NotCandidate(cell, known) => write!(f, "{} cannot be solved with {}", cell, known),
            Error::AlreadySolved(cell, known, current) => write!(f, "{} cannot be changed from {} to {}", cell, current, known),
